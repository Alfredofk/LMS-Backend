...
const { runInSchool } = require('./tenantContext');
...
function signAccessToken({ userId, membershipId, schoolId, roles }) {
    return jwt.sign(
        { sub: userId, membershipId: membershipId ?? null, schoolId: schoolId ?? null, roles: roles ?? [] },
...
function requireAuth(req, res, next) {
    ...
    return runInSchool(req.auth.schoolId, () => next());
}
